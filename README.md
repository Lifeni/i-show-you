...
Before performing the following steps, please make sure that the newer Docker and Docker Compose are installed on the machine. Currently the image only supports the amd64 version of Linux.
...
1. Download the [docker-compose.yml](https://github.com/Lifeni/i-show-you/blob/master/build/docker-compose.yml) file in the repo to your own machine. It is best to create a separate `folder` and put the files in it.
   ...
   [Click here to download](https://raw.githubusercontent.com/Lifeni/i-show-you/master/build/docker-compose.yml).
...
4. Open [http://localhost:8080](http://localhost:8080) or reverse proxy port 8080 to view the front-end page.
...
![File Editing Page](https://file.lifeni.life/dashboard/i-show-you/1.webp)
...
![Login to Admin Page](https://file.lifeni.life/dashboard/i-show-you/7.webp)
...
3. After configuring Nginx (or Caddy) and MongoDB, run the Go program.
   ...
   Don’t forget the dot at the end of the command. If it can run normally, there will be an Echo character drawing Logo and the words `Connected to database`. Then open the address set in the Nginx (or Caddy) configuration (if you use the configuration file I provided, the default address is http://localhost, which is the local port 80).
